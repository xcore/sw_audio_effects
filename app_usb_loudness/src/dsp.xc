...
static void get_samples_from_host(
	...
	int gain_iterations // No of times to use non-linear-gain processing
)
{
	int chan_cnt; // channel counter
...
#pragma loop unroll
	for (chan_cnt=0; chan_cnt<NUM_USB_CHAN_OUT; chan_cnt++)
	{
		samples[chan_cnt] = non_linear_gain_wrapper( l_samp[chan_cnt] ,chan_cnt ,gain_iterations );
...
)
{
	...
	int gain_iterations = 4; // No of times to use non-linear-gain processing
	...
	while (1) 
	{
		...
		else 
		{
			...
			get_samples_from_host( c_aud_in ,out_samples ,sampFreq ,gain_iterations );
